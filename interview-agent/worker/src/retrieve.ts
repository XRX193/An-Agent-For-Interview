...
export async function retrieveRelevantDocs(question: string, env: WorkerEnv, scope?: string) {
  const gh = env.GITHUB_USERNAME ?? 'XRX193'
  const projectsPromise = listProjects(env.SUPABASE_URL, env.SUPABASE_ANON_KEY, gh).catch(() => [] as Project[])
  ...
  if (embedding) {
    const docs = await searchSimilarDocs(embedding, { limit: 7, matchThreshold: 0.3, scope, url: env.SUPABASE_URL, key: env.SUPABASE_ANON_KEY, githubUser: gh })
  ...
  const docs = await searchByKeywords(question, { limit: 5, scope, url: env.SUPABASE_URL, key: env.SUPABASE_ANON_KEY, githubUser: gh })
