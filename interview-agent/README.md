...
[![Deploy Frontend](https://github.com/{{github_username}}/interview-agent/actions/workflows/deploy-frontend.yml/badge.svg)](https://github.com/{{github_username}}/interview-agent/actions/workflows/deploy-frontend.yml)
[![Deploy Worker](https://github.com/{{github_username}}/interview-agent/actions/workflows/deploy-worker.yml/badge.svg)](https://github.com/{{github_username}}/interview-agent/actions/workflows/deploy-worker.yml)
...
编辑 `config.json`，将 `github_username` 改为你的 GitHub 用户名：
...
2. 在 SQL Editor 中执行建表语句（见 [`indexer/upsert.py`](indexer/upsert.py) 中的 `init_supabase_tables` SQL）
...
打开 `https://{你的用户名}.github.io/interview-agent`，尝试发送一个测试问题。

## 项目结构

```
interview-agent/
├── frontend/          # React 18 + TypeScript 前端（部署到 GitHub Pages）
│   └── src/
│       ├── components/    # UI 组件
...
MIT © [你的名字](https://github.com/你的用户名)

---

> 📖 详细需求分析和实现路径见：[面试Agent需求与实现路径.md](../面试Agent需求与实现路径.md)
